...
14. **Roll automatique des dates (décision Julien 2026-09-21), seule exception à l'application humaine des données factuelles.** La routine cloud `agoris-roll-2027-watch` merge ses propres PR de handoffs sans validation humaine, et `.github/workflows/roll-apply.yml` applique ensuite en base les handoffs `roll-pret`, sans relecture. Conditions non négociables : (a) l'agent qui navigue sur les sites tiers ne détient JAMAIS de clé d'écriture, la clé service-role vit dans les secrets GitHub ; (b) l'écriture est faite par un script déterministe (`scripts/roll-apply-handoffs.ts`) qui ne touche que `start_date`, `end_date`, `edition_year` ; (c) chaque handoff passe les garde-fous de `scripts/roll-guards.ts` (pas de changement de ville ou de lieu, écart cohérent avec `frequency`, source officielle relue par le script avec les dates retrouvées sur la page) ; (d) tout rejet part en arbitrage humain via `tasks/roll-2027/journal.json` ; (e) le MDX et le SEO ne sont jamais roulés automatiquement, ils passent par le process writer + reviewers (file : `roll-apply-handoffs.ts --queue`). Ne pas étendre cette exception à d'autres champs ou à d'autres flux sans décision explicite.
