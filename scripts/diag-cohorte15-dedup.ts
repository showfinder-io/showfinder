...
type Row = { slug: string; status: string; editorial_mdx: string | null; edition_year: number | null; organizer_name: string | null };
...
async function main() {
  ...
  for (const r of rows) console.log(`${r.slug.padEnd(40)} status=${r.status} mdx=${r.editorial_mdx ? (r.editorial_mdx.length + "c") : "NULL"} ed=${r.edition_year} org=${r.organizer_name}`);
  console.log(`(${rows.length}/9 trouvées, ${rows.filter((r) => r.editorial_mdx).length} avec editorial_mdx)`);
  ...
  if (APPLY && (dup ?? []).length) {
    ...
    const tgt = (target ?? []) as { slug: string; editorial_mdx: string | null }[];
    console.log("DELETE exécuté. pcd-congress restant:", (after ?? []).length, "| cible adf-pcd-paris présente:", tgt.length, "mdx:", tgt[0]?.editorial_mdx?.length + "c");
