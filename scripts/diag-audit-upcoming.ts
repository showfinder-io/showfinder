 ...
 *
 * Usage : set -a && source .env.local && set +a && ./node_modules/.bin/tsx scripts/diag-audit-upcoming.ts
 */
...
const SYSTEM = `Tu vérifies si un salon enregistré dans un annuaire existe réellement tel que décrit, à partir du contenu de sa page officielle. On te donne nom/ville/dates enregistrés + le contenu de la page.
Réponds UNIQUEMENT en JSON : {"verdict":"ok|suspect|phantom","reason":"...","confidence":"high|medium|low"}.
- "ok" : la page confirme un salon de ce nom, dans cette ville, à une période proche (même mois et année, ou édition à venir cohérente).
- "suspect" : doute réel (dates ou ville divergentes, page ambiguë, marque peut-être absorbée/renommée, page sans info claire sur l'édition).
- "phantom" : la page contredit clairement (autre ville, autre marque, salon disparu/annulé/fusionné, ou aucune trace du salon).
Sois prudent : au moindre doute sérieux d'inexistence ou de mauvaise localisation, "suspect" plutôt que "ok".`;
...
async function main() {
  ...
  const { data } = await sb.from("salons").select("slug,name,city,start_date,end_date,edition_year,website_url,editorial_mdx").eq("status", "published").gte("start_date", today).order("start_date");
  const rows = data ?? [];
  ...
  const path = join(OUT, `audit-upcoming-${today}.csv`);
